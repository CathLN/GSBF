 ...
 * @return un tableau contenant l'id, le nom et le pr�nom sous la forme d'un tableau associatif 
*/
 ...
 * @param $idVisiteur 
 * @return un tableau associatif de cl� un mois -aaaamm- et de valeurs l'année et le mois correspondant 
*/
 ...
 */
	...
	public function majEtatFicheFrais($idVisiteur,$mois,$etat){
        ...
        public function clotureLesFiches($mois){
            $lesVisiteurs = $this->getLesVisiteurs();
            
            $moisACreer = getMoisSuivant($mois);
            foreach ($lesVisiteurs as $unVisiteur){
                $idvisiteur = $unVisiteur['id'];
                $laDerniereFiche = $this->getLesInfosFicheFrais($idvisiteur,$mois);
                if($laDerniereFiche['idEtat']=='CR'){
		...
		$req = "insert into fichefrais(idvisiteur,mois,nbJustificatifs,montantValide,dateModif,idEtat) 
		values('$idvisiteur','$moisACreer',0,0,now(),'CR')";
		PdoGsb::$monPdo->exec($req);
		$lesIdFrais = $this->getLesIdFrais();
		foreach($lesIdFrais as $uneLigneIdFrais){
			$unIdFrais = $uneLigneIdFrais['idfrais'];
        ...
        public function majFicheFrais($idVisiteur,$mois,$etat,$montantValide){
 ...
 */
        public function miseAjourFraisHF($lesFraisHF){
            ...
            if ($montantValide != 0) {
                $montantValide += $this->getCumulFraisFF($idVisiteur,$mois);
                $this->majFicheFrais($idVisiteur,$mois,'VA',$montantValide);
                echo $montantValide;
            }
        }
        
        
        public function getCumulFraisFF($idVisiteur,$mois){
